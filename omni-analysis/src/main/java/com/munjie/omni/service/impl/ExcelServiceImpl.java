...
public class ExcelServiceImpl implements ExcelService {
    ...
    @Override
    public File createExcelFile(List<ScoreEntity> allScore, List<QuestionAnalysisEntity> allQuestion,String title) {
        // 分组
        Map<String, List<ScoreEntity>> stringListMap = allScore.stream()
        ...
        for (Map.Entry<String, List<ScoreEntity>> stringListEntry : stringListMap.entrySet()) {
            ...
            // 创建工作簿
            sheet = workbook.createSheet(lesson + "质量分析报表");
            // 设置字体样式
            ...
            return PoiUtils.createExcelFile(workbook, title + "质量分析表");
        } catch (IOException e) {
            log.error("创建质量分析报告异常", e);
            return null;
        }
    }
